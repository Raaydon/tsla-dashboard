...
class Tesla(OAuth2Session):
    ...
    def _token_updater(self, token=None):
        ...
        # Read token from cache
            ...
            if 0 < self.expires_at < time.time():
                ...
                logger.debug(
                    "Cached SSO token expires at %s", time.ctime(self.expires_at)
                )
    ...
    def battery_list(self):
        ...
        return [
            Battery(p, self)
            for p in self.api("PRODUCT_LIST")["response"]
...
class Vehicle(JsonDict):
    ...
    def stream(self, callback=None, retry=0, indefinitely=False, **kwargs):
        ...
        **kwargs: Optional arguments that `run_forever` takes.
        ...
        websocket.enableTrace(
            logger.isEnabledFor(logging.DEBUG), handler=logging.NullHandler()
        )
        wsapp = websocket.WebSocketApp(
            STREAMING_BASE_URL + "streaming/",
        ...
        if not self.available():
            ...
            while True:
                ...
                if start_time + timeout - interval < time.time():
                    raise VehicleError(
                        "%s not woken up within %s seconds"
    ...
    def option_code_list(self):
        ...
        return list(
            filter(
                None,
                [self.decode_option(code) for code in self["option_codes"].split(",")],
            )
        )
    ...
    def get_service_scheduling_data(self):
        ...
        return next(
            (
                enabled
                for enabled in response["enabled_vins"]
    ...
    def compose_image(self, view="STUD_3QTR", size=640, options=None):
        ...
        if options is None:
            logger.warning(
                "`compose_image` requires `options` to be set for " "an accurate image"
            )
        ...
        response = requests.get(
            url, params=params, verify=self.tesla.verify, proxies=self.tesla.proxies
        )
        response.raise_for_status()  # Raise HTTPError, if one occurred
...
class Battery(Product):
    ...
    def set_backup_reserve_percent(self, percent):
        ...
        return self.command("BACKUP_RESERVE", backup_reserve_percent=int(percent))
